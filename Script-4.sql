...
--SELECIONE O SELECT A BAIXO PARA VISUALISAR OS USUARIOS E ANALISTAS

SELECT *FROM USUARIO U 
SELECT *FROM ANALISTA A 

--NESTE INSERT A BAIXO INSERIMOS NOVAS SOLICITACOES PARA SER ENVIADA PARA ANALISE DE UM ANALISTA
INSERT INTO SOLICITACAO (DATA_CRIACAO, STATUS, USUARIO_ID, ANALISTA_ID, PRODUTO_ID)
VALUES (now(),'Em processamento', 3, 20, 3);

--NO SELECT A BAIXO BUSCAMOS AS SOLICITACOES, DA MAIS ANTIGA PARA A MAIS RECENTE, PARA UM ANALISTA AVALIAR
...
-- APOS AVALIACAO, O ANALISTA APROVA OU RECUSA A SOLICITACAO
-- OBJETIVO SIMULAR UMA RESOLUCAO DO ANALISTA E ATUALIZAR O STATUS DA SOLICITACAO
UPDATE SOLICITACAO
SET ATUALIZACAO_DATA = NOW(),
STATUS = 'APROVADO',
DATA_FINALIZACAO = NOW()
WHERE SOLICITACAO_ID =8;

--VISUALISANDO O STATUS DO USUARIO
SELECT 
USUARIO_ID 
FROM SOLICITACAO S
WHERE SOLICITACAO_ID = 8;

-- CASO SOLICITE UM NOVO CARTAO, RODAMOS O SELECT A BAIXO PARA ENVIAR AS INFORMACOES DE USUARIO PARA O ANALISTA 
--CASO NAO DESEJE APROVAR O CARTAO RODAR O PROXIMO SELECT
...
-- VISAO GERAL DO PROCESSO, NA QUAL PODEMOS VER OS DADOS EM TODAS AS TABELAS DO FLUXO DE ANALISE
SELECT * FROM USUARIO U
LEFT JOIN CARTAO C 
ON C.USUARIO_ID = U.USUARIO_ID
LEFT JOIN SOLICITACAO S 
ON S.SOLICITACAO_ID = C.SOLICITACAO_ID 
LEFT JOIN ANALISTA A 
ON A.ANALISTA_ID = S.ANALISTA_ID
LEFT JOIN PRODUTO P 
ON P.PRODUTO_ID = S.PRODUTO_ID
WHERE U.USUARIO_ID = 3;

--RODE O SELECT A BAIXO PREENCHENDO O ID NO WHERE PARA VISUALIZAR A QUAL USUARIO O PRODUTO CARTAO PERTENCE.
